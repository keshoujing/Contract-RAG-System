...
| [retrieval_eval.md](retrieval_eval.md) | 2026-06-08 检索接通 `POST /api/query` + RAGAS 评测基线（2026004 scoped：faithfulness 0.86/answer_correctness 0.44 等）+ 模型分层 + ⚠️RAGAS judge 不能用 gemini-3-flash-preview（list-content 致 faithfulness=0）+ 七奠基石 baseline-vs-agentic 对比（困难混合/跨合同题上 agent retrieval_coverage 0.30→0.77、source_precision 0.60→1.00、answer_sim +0.07，问题越难差距越大；切端点的权威证据）+ 接地校验闸 grounding guard（2026-06-22：clause 精确子串闸/record 回查 ledger 重投影/无依据则拒答，`retrieval/grounding.py`，服务端强制可追溯）+ prompt-injection 防御（2026-06-22：检索文本 spotlight 数据框 + system prompt 安全规则，`retrieval/injection.py`，eval `evals/run_injection.py`）。开发检索/问答前先读 |
